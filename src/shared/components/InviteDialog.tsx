...
export function InviteDialog({ open, onOpenChange, workspaceId, userPermissions = {} }: InviteDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl bg-[#2d3250] max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle className="text-2xl text-white font-semibold">Invite to this workspace</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 mt-6 overflow-y-auto flex-1">
                  ...
                  >
                    ...
                    >
                      ...
                      <SelectContent className="bg-[#2d3250] border-[#404463]">
                        <SelectItem value="projectmanager" className="text-white">Workspace Project Manager</SelectItem>
                        <SelectItem value="admin" className="text-white">Workspace Architect</SelectItem>
                        <SelectItem value="client" className="text-white">Workspace Client</SelectItem>
                        <SelectItem value="developer" className="text-white">Workspace Developer</SelectItem>
                        <SelectItem value="viewer" className="text-white">Workspace Viewer</SelectItem>
                      </SelectContent>
                    </Select>
                    <button
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-white">People in this workspace</h3>
            
            {filteredMembers.length === 0 ? (
              <p className="text-gray-400 text-center py-8">
                {searchQuery.trim() ? "No members found" : "No members in this workspace"}
              </p>
            ) : (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-400 capitalize">
                          {member.role === 'owner' ? 'Workspace Owner' :
                           member.role === 'admin' ? 'Workspace Architect' : 
                           member.role === 'client' ? 'Workspace Client' : 
                           member.role === 'developer' ? 'Workspace Developer' :
                           member.role === 'viewer' ? 'Workspace Viewer' : 
                           member.role}
                        </p>
                      </div>
                      <Button
                        ...
                        variant="ghost"
                        className={`text-gray-400 hover:text-white disabled:opacity-50 ${!canRemoveMembers ? 'cursor-not-allowed' : 'disabled:cursor-not-allowed'}`}
                      >
                        Remove
                      </Button>
                    </div>
