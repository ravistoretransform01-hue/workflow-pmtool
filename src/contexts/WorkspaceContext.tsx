...
export const WorkspaceProvider = ({ children }: { children: ReactNode }) => {
  ...
  const loadWorkspaces = async () => {
    ...
    try {
      ...
      const workspacesData = [
        ...(ownedWorkspaces || []),
        ...additionalWorkspaces.filter(ws => !ownedWorkspaces?.find(ows => ows.id === ws.id))
      ];
      ...
      const { data: foldersData, error: foldersError } = await supabase
        .from('folders')
        .select('*')
        .in('workspace_id', workspacesData.map(ws => ws.id));
      ...
      const workspacesWithData: Workspace[] = workspacesData.map(ws => {
        const wsFolders = (foldersData || []).filter(f => f.workspace_id === ws.id);
        const wsBoards = (boardsData || []).filter(b => b.workspace_id === ws.id && !b.folder_id);
        ...
        return {
          ...
          folders: wsFolders.map(f => {
            const folderBoards = (boardsData || []).filter(b => b.folder_id === f.id);
            return {
              id: f.id,
              name: f.name,
  ...
  const addWorkspace = async (workspace: Omit<Workspace, 'id' | 'folders' | 'boards'>, members?: { test_user_id: string; role: string }[], rolePermissions?: Record<string, Record<string, boolean>>): Promise<string | null> => {
    try {
      ...
      if (members && members.length > 0) {
        const additionalMembers = members.filter(member => member.test_user_id !== currentUser.id);
        ...
        if (additionalMembers.length > 0) {
          ...
          if (additionalMembersError) {
            console.error('Error adding additional workspace members:', additionalMembersError);
          }
        }
      }
      ...
      if (rolePermissions) {
        const defaultRoles = ['owner', 'projectmanager', 'admin', 'client', 'developer', 'viewer'];
        ...
        if (customRoleEntries.length > 0) {
          const permissionInserts = customRoleEntries.map(([roleName, permissions]) => ({
            workspace_id: data.id,
            role_name: roleName,
  ...
  const addBoard = async (workspaceId: string, board: Omit<Board, 'id' | 'url'>, folderId?: string, members?: Array<{ test_user_id: string; role: string }>) => {
    try {
      const { data, error } = await supabase
        .from('boards')
        .insert({
      ...
      if (members && members.length > 0) {
        ...
        const nonCreatorMembers = members.filter(m => m.test_user_id !== currentUser.id);
  ...
  const updateFolderName = async (workspaceId: string, folderId: string, newName: string) => {
    try {
  ...
  const updateBoardName = async (workspaceId: string, boardId: string, newName: string) => {
    try {
      const { error } = await supabase
        .from('boards')
        .update({ name: newName })
  ...
  const deleteWorkspace = async (workspaceId: string) => {
    try {
      ...
      localStorage.setItem('deleted-items', JSON.stringify([deletedItem, ...deletedItems]));
      ...
      setWorkspaces((prev) =>
        prev.map((ws) => {
          ...
          if (sourceFolderId === null) {
            ...
            const sourceFolder = ws.folders.find((f) => f.id === sourceFolderId);
  ...
  const reorderBoards = (workspaceId: string, folderId: string, newOrder: Board[]) => {
    setWorkspaces((prev) =>
      prev.map((ws) =>
        ws.id === workspaceId
          ? {
              ...ws,
              folders: ws.folders.map((folder) =>
                folder.id === folderId ? { ...folder, boards: newOrder } : folder
              ),
            }
          : ws
      )
    ...
    try {
      ...
      setWorkspaces((prev) => {
        ...
        if (sourceFolderId === null) {
          ...
          const sourceFolder = sourceWs?.folders.find(f => f.id === sourceFolderId);
          board = sourceFolder?.boards.find(b => b.id === boardId);
        }
        ...
        return prev.map((ws) => {
          ...
          if (ws.id === sourceWorkspaceId) {
            return {
              ...ws,
              boards: sourceFolderId === null 
                ? ws.boards.filter(b => b.id !== boardId)
                : ws.boards,
              folders: sourceFolderId !== null
  ...
  const duplicateBoard = async (
    ...
    try {
      ...
      if ((option === 'structure-items' || option === 'structure-items-updates') && Object.keys(groupIdMap).length > 0) {
        ...
        if (option === 'structure-items-updates' && Object.keys(itemIdMap).length > 0) {
  ...
  return (
    <WorkspaceContext.Provider value={{ workspaces, addWorkspace, addBoard, addFolder, updateFolderName, updateBoardName, updateWorkspaceName, deleteWorkspace, deleteBoard, deleteFolder, moveBoard, moveBoardToWorkspace, getWorkspaceById, reorderFolders, reorderBoards, duplicateBoard }}>
      {children}
